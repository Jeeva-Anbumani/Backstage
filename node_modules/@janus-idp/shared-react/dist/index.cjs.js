...
const HorizontalStackedBars = ({
  ...
  "div",
  {
    ...
    className: classNames__default["default"]("bs-shared-horizontal-stacked-bars", {
      "is-inline": inline
    }),
...
const CamelCaseWrap = ({ value, dataTest }) => {
  ...
  const rendered = /* @__PURE__ */ React__default["default"].createElement("span", { "data-testid": dataTest }, words == null ? void 0 : words.map((word, i) => /* @__PURE__ */ React__default["default"].createElement(React__default["default"].Fragment, { key: word }, word, i !== words.length - 1 && /* @__PURE__ */ React__default["default"].createElement("wbr", null))));
  MEMO[value] = rendered;
  return rendered;
};
...
const math = mathjs.create({ createUnitDependencies: mathjs.createUnitDependencies, unitDependencies: mathjs.unitDependencies });
...
const TaskStatusTooltip = ({ taskStatus }) => {
  return /* @__PURE__ */ React__default["default"].createElement("div", { className: "bs-shared-task-status-tooltip" }, Object.keys(ComputedStatus).map((status) => {
    const { message, color } = getRunStatusColor(status);
    return taskStatus[status] ? /* @__PURE__ */ React__default["default"].createElement(React__default["default"].Fragment, { key: status }, /* @__PURE__ */ React__default["default"].createElement(
      "div",
      {
        className: "bs-shared-task-status-tooltip__legend",
        style: { background: color }
      }
    ), /* @__PURE__ */ React__default["default"].createElement("div", null, status === ComputedStatus.PipelineNotStarted || status === ComputedStatus.FailedToStart ? message : `${taskStatus[status]} ${message}`)) : null;
  }));
};
...
const StatusIconAndText = ({
  ...
  return /* @__PURE__ */ React__default["default"].createElement(
    "span",
    {
      className: classNames__default["default"]("bs-shared-icon-and-text", className),
    ...
    /* @__PURE__ */ React__default["default"].createElement(CamelCaseWrap, { value: title, dataTest: "status-text" })
  );
};

const useDeepCompareMemoize = (value, stringify) => {
  const ref = React__default["default"].useRef();
  if (stringify ? JSON.stringify(value) !== JSON.stringify(ref.current) : !lodash.isEqual(value, ref.current)) {
    ref.current = value;
  }
  return ref.current;
};
...
const useDebounceCallback = (callback, timeout = 500, debounceParams = {
  ...
  const callbackRef = React__default["default"].useRef();
...
exports.TaskStatusTooltip = TaskStatusTooltip;
exports.TerminatedReasons = TerminatedReasons;
exports.cancelledColor = cancelledColor;
exports.downloadLogFile = downloadLogFile;
...
exports.pendingColor = pendingColor;
exports.pipelineGroupColor = pipelineGroupColor;
exports.pipelineRunFilterReducer = pipelineRunFilterReducer;
exports.pipelineRunStatus = pipelineRunStatus;
exports.runningColor = runningColor;
exports.skippedColor = skippedColor;
